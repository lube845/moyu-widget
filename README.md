...
无边框、置顶、可拖动的 Windows 桌面小组件。鼠标横向滑动翻页，六个页面：
...
- **待办事项** — 自管理清单：新增待办、勾选完成、删除、清除已完成；未完成项始终在顶部，已完成项按完成顺序排到底部；进度条 + 进度百分比；本地持久化
...
<p>
  ...
  <img src="docs/screenshots/3-todo.png" width="140" alt="待办事项页" />
...
- **横向滑动翻页**：在卡片内**左键按住横向拖动 40px 以上**，或在**触控板上双指横向扫**，或**滚动鼠标滚轮（上滚 = 上一页，下滚 = 下一页）**，即可在 6 页之间循环切换（向右拖 = 上一页，向左拖 = 下一页）
- **点底部指示器翻页**：底部中央有 6 个小圆点 + 左右箭头，当前页对应的点会变成长条；点箭头可翻页，点对应的小点也可直接跳到该页
...
- 标题「待办事项」+ 状态标签（`N 项未完成` / `暂无`）
- 顶部输入框 + `添加` 按钮，新增待办；输入为空时按钮置灰；回车直接提交
- 列表中点左侧圆形复选框切换完成态；悬浮出现 × 删除单条
- **顺序守恒**：未完成项始终在顶部，已完成项按完成顺序排列在底部（在分组末端追加）
- 底部进度条：完成占比 + ✅ emoji 鱼 + 百分比 + caption「共 N 项 · 已完成 M 项」
- 数据持久化在 `localStorage['moyu-todo-list-v1']`，跨会话保留
...
src/
├── App.tsx              # 卡片 UI + 横向滑动翻页 + 6 页路由（下班/发工资进度条编辑）
├── styles.css           # 卡片样式（鱼缸、杯子、扭蛋、一言、黄历、待办、翻页动画、布局）
├── main.tsx             # React 渲染入口
├── components/
...
│   └── TodoList.tsx        # 待办事项页（输入 / 完成 / 删除 / 排序守恒 / 持久化）
└── lib/
...
| 待办事项 | `localStorage['moyu-todo-list-v1']`（id / text / done，跨会话保留） |
...
- **配色**：暖白卡片 `#FBF6EC` + 抹茶绿 `#5B7F5A` + 杏橙 `#E08E45`，强调元素按页面主题切换色（日历橙+金+绿 / 黄历绿+橙 / 待办绿 / 饮水蓝 / 扭蛋橙+绿 / 一言橙+棕）
- **尺寸**：固定 `280 × 300 px`；卡片底部居中是翻页指示器（6 个圆点 + 左右箭头），当前页对应的点变成长条
- **拖动区域**：卡片顶部 10px 是 Electron `-webkit-app-region: drag` 专用拖窗口；其余区域用来响应横向滑动翻页 —— 避免鼠标拖动事件和窗口拖动冲突
- **布局**：一言页用 `position: absolute` 精确填满卡片内容区；其余五页用 `height: 100%` + flex 子项 `margin: auto 0` 吃掉剩余垂直空间，让主体垂直居中、caption 落底
